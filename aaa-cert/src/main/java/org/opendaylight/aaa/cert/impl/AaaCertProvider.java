...
public class AaaCertProvider implements IAaaCertProvider {
    ...
    public boolean addCertificateODLKeyStore(final String storePasswd, final String alias, final String certificate) {
        KeyStore keyStore = odlKeyTool.addCertificate(odlKeyTool.loadKeyStore(ctlKeyStore.getName(), storePasswd), certificate, alias, true);
    ...
    public boolean addCertificateTrustStore(final String storePasswd, final String alias, final String certificate) {
        KeyStore keyStore = odlKeyTool.addCertificate(odlKeyTool.loadKeyStore(trustKeyStore.getName(), storePasswd), certificate, alias, true);
    ...
    private boolean createTrustKeyStore() {
        KeyStore keyStore = odlKeyTool.createEmptyTrustKeyStore(trustKeyStore.getStorePassword());
        return odlKeyTool.exportKeystore(keyStore, trustKeyStore.getStorePassword(), trustKeyStore.getName());
    }

    private boolean createODLKeyStore() {
        KeyStore keyStore = odlKeyTool.createKeyStoreWithSelfSignCert(ctlKeyStore.getName(), ctlKeyStore.getStorePassword(), ctlKeyStore.getDname(),
                ctlKeyStore.getAlias(), ctlKeyStore.getValidity(), ctlKeyStore.getKeyAlg(), ctlKeyStore.getKeysize(), ctlKeyStore.getSignAlg());
        return odlKeyTool.exportKeystore(keyStore, ctlKeyStore.getStorePassword(), ctlKeyStore.getName());
    }

    @Override
    public boolean createKeyStores() {
        return createODLKeyStore() && createTrustKeyStore();
    }
    ...
    public String[] getCipherSuites() {
        ...
        if (ctlKeyStore.getCipherSuites() != null & !ctlKeyStore.getCipherSuites().isEmpty()) {
            suites = new String[ctlKeyStore.getCipherSuites().size()];
            for(int i = 0 ; i < ctlKeyStore.getCipherSuites().size() ; i++) {
                suites[i] = ctlKeyStore.getCipherSuites().get(i).getSuiteName();
            }
        }
        return suites;
    }
