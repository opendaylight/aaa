...
public class MdsalUtilsTest {
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        final CheckedFuture<Optional<DataObject>, ReadFailedException> cfr = mock(CheckedFuture.class);
        when(cfr.checkedGet()).thenReturn(dataObjectOptional);
        when(cfr.get()).thenReturn(dataObjectOptional);
        final ReadOnlyTransaction rot = mock(ReadOnlyTransaction.class);
        when(rot.read(any(), any())).thenReturn(cfr);

        final CheckedFuture<Void, TransactionCommitFailedException> cfw = mock(CheckedFuture.class);
        final WriteTransaction wot = mock(WriteTransaction.class);
        when(wot.submit()).thenReturn(cfw);

        final DataBroker dataBrokerInit = mock(DataBroker.class);
        when(dataBrokerInit.newReadOnlyTransaction()).thenReturn(rot);
        when(dataBrokerInit.newWriteOnlyTransaction()).thenReturn(wot);
    ...
    @Test
    public void initalizeDatastoreTest() {
        MdsalUtils.initalizeDatastore(LogicalDatastoreType.CONFIGURATION, dataBroker, instanceIdentifier, sslData);
    }
}
