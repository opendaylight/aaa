...
public final class WhiteboardWebServer implements WebServer {
    ...
    @VisibleForTesting
    static String checkSymbolicName(final String name) {
        if (OSGI_SYMBOLIC_NAME.matcher(name).matches()) {
            return name;
        }
        throw new IllegalArgumentException("WebContext name '%s' is not a symbolic-name".formatted(name));
    }
}
