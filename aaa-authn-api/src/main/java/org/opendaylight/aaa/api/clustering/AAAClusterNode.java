...
public class AAAClusterNode extends Thread{
    ...
    private AAAClusterConnection[] connections = new AAAClusterConnection[0];
    private Map<String,Integer> connIndex = new HashMap<>();
    ...
    private void addConnection(AAAClusterConnection c) throws UnknownHostException {
        synchronized (this.connections){
            ...
            if(nullPosition!=-1){
                ...
                AAAClusterConnection temp[] = new AAAClusterConnection[connections.length+1];
            ...
            try {
                ...
                if(fitPattern) {
                    ...
                    synchronized(connections){
                        Integer connectionIndex = connIndex.get(connectionKey);
                        if(connectionIndex==null){
                            shouldAddConnection = true;
                        }else{
                            //Wait for 2 seconds to make sure a connection is not being added
                            Thread.sleep(2000);
                            if(connections[connectionIndex]==null || !connections[connectionIndex].isAlive()){
                                shouldAddConnection = true;
                            }
                        }
                    }
    ...
    private void sendObject(Object o,int op){
        for(int i=0;i<connections.length;i++){
            if(connections[i]!=null) {
                try {
                    //We might get an NPE here if the connection was shutdown while iterating.
                    switch (op) {
                        case AAAObjectEncoder.OPERATION_WRITE:
                            connections[i].writeObject(o);
                            break;
                        case AAAObjectEncoder.OPERATION_UPDATE:
                            connections[i].updateObject(o);
                            break;
                        case AAAObjectEncoder.OPERATION_DELETE:
                            connections[i].deleteObject(o);
                            break;
                    }
                    break;
                } catch (Exception e) {
                    if(!(e instanceof NullPointerException)){
                        LOGGER.error("Failed to send to host ", e);
                    }
                }
            }
        }
