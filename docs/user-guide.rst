...
-  Oauth2ProxyTokenRealm
   ...
   - Disabled out of the box. See :ref:`configuring-oauth2-proxy-token-realm`.
...
The ``Oauth2ProxyTokenRealm`` enables OpenDaylight to delegate authentication
...
The ``Oauth2ProxyTokenFilter`` (a Shiro ``AuthenticatingFilter``) intercepts
each incoming request, reads these headers, and builds an
``Oauth2ProxyToken``. Because the IdP has already validated the user, the
token carries no password credential — OpenDaylight trusts the forwarded
identity. The ``Oauth2ProxyTokenRealm`` then authenticates and authorizes the
...
.. warning::
    ...
    When ``Oauth2ProxyTokenRealm`` is active, OpenDaylight must only be
...
.. _configuring-oauth2-proxy-token-realm:

Configuring Oauth2ProxyTokenRealm
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code-block:: xml
    ...
    <main>
        <pair-key>oauth2ProxyTokenRealm</pair-key>
        <pair-value>org.opendaylight.aaa.shiro.realm.Oauth2ProxyTokenRealm</pair-value>
    </main>

    <!-- Shiro filter wired to Oauth2ProxyTokenRealm -->
    <main>
        <pair-key>oauth2ProxyTokenFilter</pair-key>
        <pair-value>org.opendaylight.aaa.shiro.filters.Oauth2ProxyTokenFilter</pair-value>
    </main>
...
.. code-block:: xml
    ...
    <main>
        ...
        <pair-value>$oauth2ProxyTokenRealm</pair-value>
    </main>

**3. Apply the filter to URL patterns**

Replace ``authcBasic`` with ``noSessionCreation, oauth2ProxyTokenFilter`` in
...
.. code-block:: xml
    ...
    <urls>
        ...
        <pair-value>noSessionCreation, oauth2ProxyTokenFilter, roles[admin]</pair-value>
    </urls>
    ...
    <urls>
        <pair-key>/**</pair-key>
        <pair-value>noSessionCreation, oauth2ProxyTokenFilter</pair-value>
    </urls>
...
filter that handles both ``Authorization: Bearer <JWT>`` and OAuth2 Proxy
forwarded-user headers in one filter-chain step. It allows endpoints to serve
both direct JWT clients and clients authenticated through an
...
2. If the request carries an ``X-Forwarded-User`` header (set by OAuth2 Proxy
   ...
   token to ``Oauth2ProxyTokenRealm``.
...
.. code-block:: xml
    ...
    <main>
        <pair-key>securityManager.realms</pair-key>
        <pair-value>$bearerJwtRealm, $oauth2ProxyTokenRealm</pair-value>
    </main>
