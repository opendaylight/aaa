...
OpenDaylight contains these implementations:
...
without passing through OAuth2 Proxy — the filter returns HTTP ``401
Unauthorized``.

If the namespace begins with the prefix role, that prefix is stripped.
The remaining string is then mapped as the ODL role name.:
...
| ``role:admin``                   | ``admin``                        |
+----------------------------------+----------------------------------+
| ``role:odl-application:admin``   | ``odl-application:admin``        |
+----------------------------------+----------------------------------+
...
**4. Apply the filter to URL patterns**
...
.. note::
    ...
    If using a namespaced role to restrict access, include the full namespace
    (e.g., roles[my:namespace:role]).
...
.. _oauth2ProxyHeaderFilter-settings:
...
``expected-issuer``
    ...
    issuer verification. Issuer is required when ``jwks-uri`` is set and
...
.. note::
   ...
   ``exp`` claim in practice. It would only be a concern for tokens with a very short lifetime
   (e.g. 60 seconds) where strict replay prevention is required; for typical access tokens with
   a 5–15 minute expiry the 60-second tolerance is inconsequential.
...
Register ``bearerJwtRealm`` with the Shiro security manager alongside the
existing realms:
...
.. code-block:: xml
    ...
    <main>
        <pair-key>securityManager.realms</pair-key>
        <pair-value>$bearerJwtRealm</pair-value>
    </main>
...
CombinedOauth2AuthenticationFilter
----------------------------------

``CombinedOauth2AuthenticationFilter`` is a special Shiro filter that bridges
the OSGi ``filterchain.cfg`` pre-authentication pipeline with Shiro realm
authentication. It works in conjunction with ``Oauth2ProxyHeaderRealm`` and
``BearerJwtRealm``, allowing a single filter-chain entry to accept both
OAuth2 Proxy forwarded-user credentials and direct ``Authorization: Bearer``
JWT tokens.

How it works
^^^^^^^^^^^^

``CombinedOauth2AuthenticationFilter`` handles two authentication scenarios:

1. Requests already authenticated by a ``filterchain.cfg`` filter running before the
   Shiro ``AAAShiroFilter`` are passed through without any OAuth2 processing, leaving
   authentication and authorization to that filter.
2. Requests from clients authenticated
   through an `OAuth2 Proxy <https://oauth2-proxy.github.io/oauth2-proxy/>`_ (via
   forwarded-user headers) and from direct JWT clients (via ``Authorization: Bearer``)
   are both accepted within a single filter-chain step.
...
2. If the request carries an ``X-Forwarded-User`` header (set by OAuth2 Proxy
   ...
   ``Oauth2ProxyHeaderToken`` from the forwarded user and groups. Shiro routes the
   token to ``Oauth2ProxyHeaderRealm``.
...
How to enable CombinedOauth2AuthenticationFilter
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**1. Register both realms**

Add ``oauth2ProxyHeaderRealm`` alongside ``bearerJwtRealm`` so that the
...
.. code-block:: xml
    ...
    <main>
        <pair-key>securityManager.realms</pair-key>
        <pair-value>$oauth2ProxyHeaderRealm, $bearerJwtRealm</pair-value>
    </main>

**2. Register the filter**

Declare the combined filter under the name ``authcCombinedOauth2``:

.. code-block:: xml

    <main>
        <pair-key>authcCombinedOauth2</pair-key>
        <pair-value>org.opendaylight.aaa.shiro.filters.CombinedOauth2AuthenticationFilter</pair-value>
    </main>
...
Replace ``authcBasic`` with ``authcCombinedOauth2`` in the ``<urls>`` section:

.. code-block:: xml

    <urls>
        <pair-key>/**/operations/cluster-admin**</pair-key>
        <pair-value>noSessionCreation, authcCombinedOauth2, roles[admin]</pair-value>
    </urls>
    <urls>
        <pair-key>/**/v1/**</pair-key>
        <pair-value>noSessionCreation, authcCombinedOauth2, roles[admin]</pair-value>
    ...
    <urls>
        <pair-key>/**</pair-key>
        <pair-value>noSessionCreation, authcCombinedOauth2</pair-value>
    </urls>
...
CombinedOauth2AuthenticationFilter settings
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``CombinedOauth2AuthenticationFilter`` shares configuration of ``Oauth2ProxyHeaderRealm``
and ``BearerJwtRealm`` for respective parts of it's functionality.
Refer to :ref:`oauth2ProxyHeaderFilter-settings` or :ref:`bearerJwtRealm-settings` how to setup this configuration.
