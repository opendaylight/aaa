 ...
 * <p>
 * For example, assume we have a service that offers some business logic, but the service can also
 * be started and stopped ({@link Restartable} for example). We don't want to expose start() and
 * stop() to all consumers of the service. Those methods should be visible (or allowed) just for the
 * component in charge of managing the services (or the creator of the service).
 * <p>
 * {@code Restrictable} along with a factory method could be used to restrict access to privileged
 * functionality.
 * <p>
 * <strong>The usage of this class has some advantages. Avoids mixing the API for two groups of
 * people: privileged and regular users. Privileged users use a different API to perform restricted
 * operations. It also avoids a state-based API where the same sequence of method calls might work
 * or might throw an exception, depending on whether the consumer is a privileged user or a regular
 * one. </strong>
 * <p>
 * Example:
 * 
 ...
 *     // This configuration is specific for the implementation. Different implementations may be configured differently.
 *     // Make setters public.
 ...
 * 
 * <hr>
 * A restrictable object doesn't have to implement an interface to use this pattern; the object
 * itself may represent the public part. Example:
 * 
 * <pre>
 * 
...
public final class Restrictable<P, R> {
    ...
    // See com/hp/demo/pattern/restricted/api/visibility/secure/RestrictedApi.txt for more information.
