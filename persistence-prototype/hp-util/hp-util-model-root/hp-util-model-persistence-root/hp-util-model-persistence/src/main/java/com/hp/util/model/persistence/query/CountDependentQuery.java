...
/**
 * Query to count objects from the data store that match the given filter.
 * 
 ...
 */
public class CountDependentQuery<F, C> implements Query<Long, C> {

    private F filter;
    private DependentDao<?, ?, F, ?, ?, ?, C> dao;

    private CountDependentQuery(F filter, DependentDao<?, ?, F, ?, ?, ?, C> dao) {
        this.filter = filter;
        this.dao = dao;
    }
     ...
     * @return the query
     */
    public static <F, C> Query<Long, C> createQuery(F filter, DependentDao<?, ?, F, ?, ?, ?, C> dao) {
        return new CountDependentQuery<F, C>(filter, dao);
    }
