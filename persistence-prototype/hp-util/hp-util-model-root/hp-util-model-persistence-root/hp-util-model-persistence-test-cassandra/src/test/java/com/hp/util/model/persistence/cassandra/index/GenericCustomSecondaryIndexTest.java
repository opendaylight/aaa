...
public class GenericCustomSecondaryIndexTest {
    private CustomSecondaryIndex<String, Long, String> index;
    ...
    public void beforeTest() throws Exception {
        ...
        ColumnFamily<String, Long> columnFamily = new ColumnFamily<String, Long>(
                "cf_test_all_rows_secondary_index", BasicType.STRING_UTF8, BasicType.LONG,
                "Secondary index column family to keep all rows in a main column family as columns for counting and pagination");

        this.index = new GenericCustomSecondaryIndex<String, Long, String>(columnFamily, BasicType.STRING_UTF8);
        ...
        CassandraTestUtil.execute(new Query<Void, CassandraContext<Astyanax>>() {
            ...
            public Void execute(CassandraContext<Astyanax> context) throws PersistenceException {
                for (ColumnFamily<?, ?> definition : GenericCustomSecondaryIndexTest.this.index.getColumnFamilies()) {
    ...
    public void afterTest() throws Exception {
        ...
        CassandraTestUtil.execute(new Query<Void, CassandraContext<Astyanax>>() {
            @Override
            public Void execute(CassandraContext<Astyanax> context) throws PersistenceException {
                for (ColumnFamily<?, ?> columnFamily : GenericCustomSecondaryIndexTest.this.index.getColumnFamilies()) {
    ...
    protected void testIndex(CassandraContext<Astyanax> context) throws PersistenceException {
        ...
        String indexKey1 = "index key 1";
        String indexKey2 = "index key 2";
        ...
        // Test initial state
        Assert.assertEquals(0, this.index.count(indexKey1, context));
        Assert.assertEquals(0, this.index.count(indexKey2, context));

        // Test update after insert
        this.index.insert(id1, null, indexKey1, context);
        Assert.assertEquals(1, this.index.count(indexKey1, context));

        this.index.insert(id1, null, indexKey1, context);
        Assert.assertEquals(1, this.index.count(indexKey1, context));

        this.index.insert(id2, null, indexKey2, context);
        Assert.assertEquals(1, this.index.count(indexKey2, context));

        // Test update after delete
        this.index.delete(id1, indexKey1, context);
        Assert.assertEquals(0, this.index.count(indexKey1, context));

        this.index.delete(id2, indexKey2, context);
        Assert.assertEquals(0, this.index.count(indexKey2, context));

        // Test update after delete all
        this.index.insert(id1, null, indexKey1, context);
        this.index.insert(id2, null, indexKey1, context);
        Assert.assertEquals(2, this.index.count(indexKey1, context));

        this.index.insert(id3, null, indexKey2, context);
        this.index.insert(id4, null, indexKey2, context);
        Assert.assertEquals(2, this.index.count(indexKey2, context));
        ...
        // Test read
        this.index.insert(id1, denormilizedData1, indexKey1, context);
        this.index.insert(id2, denormilizedData2, indexKey1, context);
        this.index.insert(id3, denormilizedData3, indexKey1, context);
        this.index.insert(id4, denormilizedData4, indexKey1, context);
        this.index.insert(id5, denormilizedData5, indexKey1, context);

        List<Column<Long, String>> indexes = this.index.read(indexKey1, context);
        ...
        // Test read columns
        indexes = this.index.read(Arrays.asList(id1, id5, Long.valueOf(100)), indexKey1, context);
        ...
        MarkPage<Column<Long, String>> page = this.index.read(indexKey1, pageRequest, context);
