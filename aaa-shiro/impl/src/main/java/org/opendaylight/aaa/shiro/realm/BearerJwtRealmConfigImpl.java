...
public final class BearerJwtRealmConfigImpl implements BearerJwtRealmConfig {
    ...
    public @interface Configuration {
        ...
        @AttributeDefinition(description = """
            Array of expected 'aud' JWT claim values.
            Leave empty to skip audience verification.""")
        String[] expected$_$audience() default {};

        @AttributeDefinition(description = """
            Array of allowed JWS signing algorithms (e.g. RS256, RS384, RS512, ES256).""")
        ...
        @AttributeDefinition(description = """
            Expected value of the JOSE typ header (e.g. JWT as generic JSON Web Token header or at+jwt for OAuth 2.0
            access tokens per RFC 9068). Leave blank to skip typ header validation.""")
        String expected$_$type() default "JWT";
     ...
     * Type checking is enabled with default value "JWT".
     */
    @VisibleForTesting
    BearerJwtRealmConfigImpl(final @Nullable JWTProcessor<SecurityContext> jwtProcessor,
        ...
        this(jwtProcessor, userClaim, roleClaim, "JWT");
    }
    ...
    public BearerJwtRealmConfigImpl(final Configuration configuration) {
        // TODO: validate configuration before applying it
        ...
        final var timeToLiveMillis = SECONDS.toMillis(configuration.cache$_$timetolive$_$seconds());
        final var rateLimitMillis = SECONDS.toMillis(configuration.rate$_$limit$_$min$_$interval$_$seconds());
        if (timeToLiveMillis <= rateLimitMillis) {
            throw new IllegalArgumentException(
                "cache-timetolive-seconds must be greater than rate-limit-min-interval-seconds");
        }
        final var cacheRefreshTimeoutMillis = SECONDS.toMillis(configuration.cache$_$refreshtimeout$_$seconds());
    ...
    private static DefaultJWTClaimsVerifier<SecurityContext> verifier(
        ...
        return new DefaultJWTClaimsVerifier<>(audience, exactMatchBuilder.build(),
            Set.of(), audience == null ? Set.of("aud") : Set.of());
    }
