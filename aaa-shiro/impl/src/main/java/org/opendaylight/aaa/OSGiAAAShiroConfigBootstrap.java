...
public class OSGiAAAShiroConfigBootstrap implements ClusteredDataTreeChangeListener<ShiroConfiguration> {
    ...
    synchronized void activate() {
        registration = dataBroker.registerDataTreeChangeListener(
                DataTreeIdentifier.create(LogicalDatastoreType.CONFIGURATION,
                        InstanceIdentifier.create(ShiroConfiguration.class)), this);
    ...
    @Override
    public synchronized void onDataTreeChanged(@NonNull Collection<DataTreeModification<ShiroConfiguration>> changes) {
        updateInstance(Iterables.getLast(changes).getRootNode().getDataAfter());
    }

    @Holding("this")
    private void updateInstance(ShiroConfiguration config) {
