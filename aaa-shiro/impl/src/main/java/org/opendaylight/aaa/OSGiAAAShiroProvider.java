...
public class OSGiAAAShiroProvider implements TokenProvider {
    ...
    public static @interface DatastoreConfiguration {
    ...
    @Activate
    public OSGiAAAShiroProvider(DatastoreConfiguration datastoreConfiguration, @Reference DataBroker dataBroker,
                                @Reference ICertificateManager certificateManager,
                                @Reference PasswordCredentialAuth credentialAuth, @Reference HttpService httpService,
                                @Reference IIDMStore iidmStore, @Reference AuthenticationService authenticationService,
                                @Reference PasswordHashService passwordHashService,
                                @Reference ShiroConfiguration shiroConfig) {
        LOG.warn("Activate");
    ...
    private static DatastoreConfig processDatastoreConfiguration(DatastoreConfiguration datastoreConfiguration) {
        ...
        return new DatastoreConfigBuilder()
                .setStore(DatastoreConfig.Store.forName(datastoreConfiguration.storeType()).orElseThrow(
                        () -> new IllegalArgumentException("Unable to map store type config  to a known store type.")))
