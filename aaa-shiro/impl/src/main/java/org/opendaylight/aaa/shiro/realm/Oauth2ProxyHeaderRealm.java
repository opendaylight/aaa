...
public final class Oauth2ProxyHeaderRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(final AuthenticationToken token)
        ...
        // Remove possibly malicious characters from username
        final var sanitizeUser = CharMatcher.javaIsoControl().removeFrom(user).strip();
        final var odlPrincipal = ODLPrincipalImpl
            .createODLPrincipal(sanitizeUser, null, sanitizeUser, parseRoles(groups));
    ...
    static Set<String> parseRoles(final List<String> headers) {
        ...
        for (var headerValue : headers) {
            ...
            if (headerValue == null || headerValue.trim().isBlank()) {
                continue;
            }
            // Remove control characters
            final var cleanHeaderValue = CharMatcher.javaIsoControl().removeFrom(headerValue);
            // Split by comma
            final var roles = cleanHeaderValue.split(",");
