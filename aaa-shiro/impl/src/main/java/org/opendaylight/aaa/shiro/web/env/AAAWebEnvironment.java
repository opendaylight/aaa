...
public final class AAAWebEnvironment extends IniWebEnvironment implements AAAShiroWebEnvironment {
    ...
    public AAAWebEnvironment(final ShiroIni shiroConfiguration, final DataBroker dataBroker,
            ...
            final ServletSupport servletSupport, final List<BearerJwtRealmConfig> jwtRealmConfigs) {
        ...
        ClassLoaderUtils.runWithClassLoader(AAAWebEnvironment.class.getClassLoader(), () -> {
            try (var filterLoad = MDSALDynamicAuthorizationFilter.prepareForLoad(dataBroker);
                 ...
                 var jwtLoad = BearerJwtRealm.prepareForLoad(
                     jwtRealmConfigs.isEmpty() ? null : jwtRealmConfigs.getFirst())) {
                configure();
            }
        });
