...
public final class CombinedOauth2AuthenticationFilter extends BearerHttpAuthenticationFilter {
    ...
    static final String PARSED_HEADER_ATTR = CombinedOauth2AuthenticationFilter.class.getName()
        + ".PARSED_HEADER_DATA";
    ...
    protected boolean isLoginAttempt(final ServletRequest request, final ServletResponse response) {
        ...
        if (user != null) {
            request.setAttribute(PARSED_HEADER_ATTR, user);
            return true;
        }
    ...
    protected AuthenticationToken createToken(final ServletRequest request, final ServletResponse response) {
        final var user = WebUtils.toHttp(request).getAttribute(PARSED_HEADER_ATTR);
        if (user != null) {
            ...
            return new Oauth2ProxyHeaderToken(Oauth2ProxyHeaderParser.parseRolesHeader(request,
                ...
                config.allowedCharactersPattern()), user.toString());
        }
