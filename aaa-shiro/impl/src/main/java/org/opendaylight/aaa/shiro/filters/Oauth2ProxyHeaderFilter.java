...
public final class Oauth2ProxyHeaderFilter extends AuthenticatingFilter {
    ...
    protected AuthenticationToken createToken(final ServletRequest request, final ServletResponse response) {
        return new Oauth2ProxyHeaderToken(Oauth2ProxyHeaderParser.parseRolesHeader(request, config),
            Oauth2ProxyHeaderParser.parseUser(request, config));
    }

    @Override
    protected boolean onAccessDenied(final ServletRequest request, final ServletResponse response) throws Exception {
        final var user = Oauth2ProxyHeaderParser.parseUser(request, config);
        if (user != null) {
