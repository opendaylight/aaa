...
public class MoonOAuthFilter extends AuthenticatingFilter {
    ...
    @Override
    protected boolean onLoginFailure(final AuthenticationToken token, final AuthenticationException exception,
    ...
    private void error(final HttpServletResponse resp, final OAuthProblemException exception) {
        try {
            final OAuthResponse r = OAuthResponse.errorResponse(SC_BAD_REQUEST).error(exception).buildJSONMessage();
            write(resp, r);
        } catch (final IOException | OAuthSystemException ex) {
            LOG.error("Failed to write the error ", ex);
        }
