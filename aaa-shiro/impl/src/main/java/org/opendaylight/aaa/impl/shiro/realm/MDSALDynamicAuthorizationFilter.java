...
public class MDSALDynamicAuthorizationFilter extends AuthorizationFilter {
    ...
    private Optional<HttpAuthorization> getHttpAuthzContainer(final DataBroker dataBroker)
    ...
    public boolean isAccessAllowed(final ServletRequest request, final ServletResponse response,
        ...
        // tracks whether the path is matched by any authz rules-- if not, default to open
        boolean matchingPathExists = false;
        ...
        LOG.debug("{} authorized the user for access", (matchingPathExists ? "unsuccessfully" : "successfully"));
        return !matchingPathExists;
    }
}
