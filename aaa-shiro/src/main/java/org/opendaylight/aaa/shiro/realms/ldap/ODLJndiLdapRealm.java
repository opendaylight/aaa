 ...
 * <code>org.apache.shiro.realm.ldap.JndiLdapRealm</code>.
 *
 * TODO Fix the <code>getRoleNamesForUser()</code> method once groups can be
 * extracted from LDAP. This method will effectively map LDAP groups to Shiro
 * roles. This logic will be hard-coded first pass, then pushed into a model
 * later.
 *
...
public class ODLJndiLdapRealm extends JndiLdapRealm {
  ...
  protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
    ...
    // Overridden to expose important audit trail information
    try {
  ...
  protected AuthorizationInfo doGetAuthorizationInfo(
    ...
    /*
     * JndiLdapContextFactory jlcf = this.getFactory(); AuthorizationInfo ai =
     * null; try { ai = this.queryForAuthorizationInfo(principals, jlcf); }
     * catch (NamingException e) { e.printStackTrace(); } return ai;
     */
  }
  ...
  protected Set<String> getRoleNamesForUser(String username,
    ...
    // TODO fix the searchFilter & searchBase so that groups are properly
    // returned from LDAP
    ...
    // groupNames;//getRoleNamesForGroups(groupNames);
