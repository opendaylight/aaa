...
public class HSQLDBStoreTest {
    ...
    public void testCreateTempRole() throws StoreException {
        Role role = HSQLDBStore.createRole("temp", "temp domain", "Temp Testing role");
        Assert.assertEquals(true, role != null);
    }

    @Test
    public void testCreateUser() throws StoreException {
        User user = HSQLDBStore.createUser("test", "pass", "domain", "desc", "email", true, "SALT");
        Assert.assertEquals(true, user != null);
    }

    @Test
    public void testCreateGrant() throws StoreException {
        Domain d = HSQLDBStore.createDomain("sdn", true);
        Role role = HSQLDBStore.createRole("temp", "temp domain", "Temp Testing role");
        User user = HSQLDBStore.createUser("test", "pass", "domain", "desc", "email", true, "SALT");
        Grant g = HSQLDBStore.createGrant(d.getDomainid(), user.getUserid(), role.getRoleid());
    ...
    public void testUpdatingUserEmail() throws StoreException {
        UserStore us = new UserStore();
        Domain d = HSQLDBStore.createDomain("sdn", true);
        User user = HSQLDBStore.createUser("test", "pass", d.getDomainid(), "desc", "email", true,
                "SALT");
