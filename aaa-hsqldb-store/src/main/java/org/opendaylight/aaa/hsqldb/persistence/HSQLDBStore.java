    ...
    public static synchronized Connection getConnection(Connection existingConnection) throws StoreException {
        ...
        try {
            if (existingConnection == null || existingConnection.isClosed()) {
                ...
                Connection c =  org.hsqldb.jdbc.JDBCDriver.getConnection(config.getDbPath(),properties);
                Statement st = c.createStatement();
                ResultSet rs = st.executeQuery("SELECT * FROM INFORMATION_SCHEMA.SYSTEM_USERS where USER_NAME='ADMIN'");

                if(!rs.next()){
                    rs.close();
                    st.close();
                    st = c.createStatement();
                    st.execute("CREATE USER ADMIN PASSWORD admin ADMIN");
                    st.close();
                }else{
                    rs.close();
                    st.close();
