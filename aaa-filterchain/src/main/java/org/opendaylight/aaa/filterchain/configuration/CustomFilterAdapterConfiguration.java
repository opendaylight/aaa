...
public class CustomFilterAdapterConfiguration implements ManagedService {
    ...
    public void updated(final Dictionary<String, ?> properties)
        ...
        if (properties == null) {
            ...
            final Map<String, String> configurationWithPID = addPID(configuration);
            updateListeners(configurationWithPID);
        }
    }
    ...
    /**
     * Method to add PID to a provided map.
     *
     * @param configuration a properties map
     * @return copy of the configuration with inserted PID
     */
    private Map<String, String> addPID(final Map<String, String> configuration) {
        final Map<String, String> newConfiguration = new HashMap<>(configuration);
        newConfiguration.put(Constants.SERVICE_PID, CUSTOM_FILTER_ADAPTER_CONFIGURATION_PID);
    ...
    private List<Filter> convertCustomFilterList(final List<FilterDTO> customFilterList,
        ...
        final List<Filter> injectedFilters = new CopyOnWriteArrayList<>();
        ...
        if (customFilterListValue != null) {
            ...
            for (String filterClazzName : filterClazzNames) {
                if (!Strings.isNullOrEmpty(filterClazzName)) {
                    ...
                    customFilterList.add(new FilterDTO(filterClazzName, applicableConfigs));
                }
            }
        }
        ...
        for (String key : fullConfigurationKeySet) {
            try {
                ...
                if (!key.equals(CUSTOM_FILTER_LIST_KEY)) {
                    LOG.error("couldn't parse property \"{}\"; skipping", key, e);
                }
            }
        }
    ...
    public void registerCustomFilterAdapterConfigurationListener(final CustomFilterAdapterListener listener) {
        ...
        this.updateListeners(configuration);
    }
}
