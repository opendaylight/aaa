...
public class CustomFilterAdapter implements Filter, CustomFilterAdapterListener {
     ...
     */
    private volatile List<Filter> injectedFilterChain = new Vector<>();

    /**
     * Whether the filterChain has elements for optimization purposes.
     */
    private volatile boolean injectedFilterChainHasElements = false;
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response,
        ...
        if (injectedFilterChainHasElements) {
            AAAFilterChain.createAAAFilterChain().doFilter(request, response, chain, injectedFilterChain);
        } else {
    ...
    private void setInjectedFilterChain(final List<Filter> filterChain) {
        this.injectedFilterChain = filterChain;
        this.injectedFilterChainHasElements = (this.injectedFilterChain.size() > 0);
