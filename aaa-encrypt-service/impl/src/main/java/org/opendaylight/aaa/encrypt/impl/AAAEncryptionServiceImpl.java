...
import org.opendaylight.aaa.encrypt.EncryptionServiceException;
...
public final class AAAEncryptionServiceImpl implements AAAEncryptionService {
    ...
    @Override
    public String encrypt(final String data) throws EncryptionServiceException {
        ...
        try {
            ...
            throw new EncryptionServiceException("Failed to encrypt data", e);
        }
        return Base64.getEncoder().encodeToString(cryptobytes);
    }

    @Override
    public byte[] encrypt(final byte[] data) throws EncryptionServiceException {
        // We could not instantiate the encryption key, hence no encryption or
        // decryption will be done.
        if (key == null) {
            LOG.warn("Encryption Key is NULL, will not encrypt data.");
    ...
    @Override
    public String decrypt(final String encryptedData) throws EncryptionServiceException {
        ...
        final byte[] cryptobytes;
        try {
            cryptobytes = Base64.getDecoder().decode(encryptedData);
        } catch (IllegalArgumentException e) {
            throw new EncryptionServiceException("Failed to decode String to Base64", e);
        }
        ...
        try {
            ...
            throw new EncryptionServiceException("Failed to decrypt encoded data", e);
        }
        return new String(clearbytes, Charset.defaultCharset());
    }

    @Override
    public byte[] decrypt(final byte[] encryptedData) throws EncryptionServiceException {
