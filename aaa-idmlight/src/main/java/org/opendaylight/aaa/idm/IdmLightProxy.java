   ...
   private static Claim dbAuthenticate(PasswordCredentials creds,
      ...
      try {
         ...
         if (domainList.size() == 0) {
            throw new AuthenticationException("Domain :" + domainName
                  + " does not exist");
         }
         domain = domainList.get(0);
      } catch (StoreException se) {
         throw new AuthenticationException("idm data store exception :"
               + se.toString() + se);
      }
      ...
      try {
         ...
         // a User's name is not necessarily unique, so check the password for
         // each
         // matching name
