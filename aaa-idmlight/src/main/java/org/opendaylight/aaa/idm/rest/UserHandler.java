 ...
 *
 * <b>Get All Users</b>
 * <code>curl -u admin:admin http://{HOST}:{PORT}/auth/v1/users
 *
 * <b>Get A Specific User</b>
 * <code>curl -u admin:admin http://{HOST}:{PORT}/auth/v1/users/{id}</code>
 *
 * <b>Create A User</b>
 * <code>curl -u admin:admin -X POST -H "Content-type: application/json" --data-binary \@user.json http://{HOST}:{PORT}/auth/v1/users
 ...
 *
 * <b>Update A User</b>
 * <code>curl -u admin:admin -X PUT -H "Content-type: application/json" --data-binary \@user.json http://{HOST}:{PORT}/auth/v1/users/{id}
 * An example of user.json file is:
 * <code>{
 *   "name": "admin2",
 *   "password", "admin2",
...
public class UserHandler {
     ...
     * security reasons.
     */
    protected static final String REDACTED_PASSWORD = "**********";
     ...
     */
    protected static final String REDACTED_SALT = "**********";
    ...
    public Response createUser(@Context UriInfo info, User user) {
        ...
        if (userPassword.length() > IdmLightApplication.MAX_FIELD_LEN) {
        ...
        try {
            ...
            LOG.error("There was an internal error creating the user", se);
    ...
    public Response putUser(@Context UriInfo info, User user, @PathParam("id") String id) {
        ...
        try {
            ...
            LOG.error("There was an internal error updating the user", se);
    ...
    public Response deleteUser(@Context UriInfo info, @PathParam("id") String id) {
        ...
        try {
            ...
            LOG.error("There was an internal error deleting the user", se);
    ...
    private Response internalError(final String verbal, final Exception e) {
        return new IDMError(500,
                "There was an internal error deleting the user",
    ...
    private Response missingRequiredField(final String fieldName) {
        return new IDMError(400,
                String.format("{} is required to create the user account.  " +
                              "Please provide a name in your payload.", fieldName),
                              "").response();
    }
     ...
     */
    private static void prepareUserForOutput(final User user) {
