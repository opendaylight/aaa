...
import org.opendaylight.aaa.idm.model.JSRole;
...
public class RoleHandler {
   ...
   public Response getRole(@PathParam("id") String id)  {
      ...
      return Response.ok(JSRole.create(role)).build();
   }

   @POST
   @Consumes("application/json")
   @Produces("application/json")
   public Response createRole(@Context UriInfo info,JSRole role) {
      ...
      //try {
         ...
         if (role.getName()==null)
            ...
            if (role.getName().length()>UserHandler.MAX_FIELD_LEN)
               return new IDMError(400,"role name max length is :" + UserHandler.MAX_FIELD_LEN,"").response();
         
         // description   
         if (role.getDescription()==null)
            role.setDescription("");
         else
            if (role.getDescription().length()>UserHandler.MAX_FIELD_LEN)
               return new IDMError(400,"role description max length is :" + UserHandler.MAX_FIELD_LEN,"").response();

         //role = roleStore.createRole(role);
         Role r = (Role)role.write();
      //}
      //catch (StoreException se) {
        // return new IDMError(500,"internal error creating role",se.message).response();
      //} 

      return Response.status(201).entity(JSRole.create(r)).build();
   } 
   ...
   @Consumes("application/json")
   @Produces("application/json")
   public Response putRole(@Context UriInfo info,JSRole role,@PathParam("id") String id) {
      long longId=0;
      ...
      //try {
         ...
         if ((role.getName()!=null) && (role.getName().length()>UserHandler.MAX_FIELD_LEN))
            return new IDMError(400,"role name max length is :" + UserHandler.MAX_FIELD_LEN,"").response();

         // description
         if ((role.getDescription()!=null) && (role.getDescription().length()>UserHandler.MAX_FIELD_LEN))
            return new IDMError(400,"role description max length is :" + UserHandler.MAX_FIELD_LEN,"").response();
         ...
         return Response.status(200).entity(JSRole.create(r)).build();
      //}
      //catch (StoreException se) {
   ...
   public Response deleteRole(@Context UriInfo info,@PathParam("id") String id) {
      ...
      //try {
         Role role = (Role)OStore.newStorable(Role.class);//new Role();
         role.setRoleid((int)longId);
         //role = roleStore.deleteRole(role);
         role = (Role)role.delete();
         if (role==null) {
            return new IDMError(404,"role id not found :" + id,"").response();
         }
      //}
